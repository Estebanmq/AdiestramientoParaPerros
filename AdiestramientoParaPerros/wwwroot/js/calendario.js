(function ($) {
  ...
  // Configuracion del calendario por defecto
  var defaults = {
    date: null,
    weekDayLength: 1,
    prevButton: "Anterior",
  ...
  //Funcion que devuelve el primer dia del mes
  ...
  function generateMonthDOM(currentDate) {
    ...
    for (var month in settings.meses) {
      if (settings.meses.hasOwnProperty(month)) {
  ...
  function generateMonthHeaderDOM(currentDate) {
    return (
      ...
      settings.meses[currentDate.getMonth() + 1] +
      "</span>" +
  ...
  function generateWeekHeaderDOM(currentDate) {
    ...
    for (var weekDay in settings.dias) {
      if (settings.dias.hasOwnProperty(weekDay)) {
        str +=
          ...
          (typeof settings.formatWeekDay === "function"
            ...
            : settings.dias[weekDay].substring(0, settings.weekDayLength)) +
          "</div>";
      }
    }
    ...
    monthData.forEach(function (week, weekNo) {
      ...
      week.forEach(function (day, dayNo) {
        ...
        if (day.getMonth() !== currentDate.getMonth()) { 
          disabled = true;
        } else if (dayNo == 0 || dayNo == 6) {
            console.log("disabled -> " + settings.dias[dayNo]);
          disabled = true;
        } 
  ...
  $.fn.calendar = function (options) {
    ...
    if (settings.startOnMonday) {
      settings.dias = settings.alternatedias;
    }
    if (settings.min) {
